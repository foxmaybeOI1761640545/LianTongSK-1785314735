...
function syncRoute() {
  ...
  if (routeDetailId.value !== panelFocusId.value) panelFocusId.value = ''
}
...
function highlight(section) {
  ...
  globalThis.clearTimeout(manualTimer)
  manualTimer = globalThis.setTimeout(() => {
    manualActive.value = ''
  }, 1800)
}
...
function openDetail(payload) {
  ...
  panelFocusId.value = payload?.mode === 'panel-scale' ? id : ''
...
const combinedRecords = computed(() => {
  const records = [
    ...(dashboard.value?.highValueRecords ?? []),
    ...(dashboard.value?.recentFocusUsers ?? []),
  ]
...
const story = (eyebrow, title, subtitle, metrics, bullets) => ({
  eyebrow,
  title,
  subtitle,
  metrics,
...
const detailCatalog = computed(() => {
  ...
  for (const kpi of dashboard.value.kpis ?? []) {
    catalog.set(`kpi-${kpi.id}`, story(
      'EXPERIENCE KPI DRILLDOWN',
      kpi.label,
      kpi.definition,
      [
        metric('当前值', kpi.displayValue, '当前筛选周期的同源聚合或规则结果'),
        metric('筛选周期', rangeLabel, '详情与驾驶舱筛选保持同步'),
        metric('客群聚焦', filters.value.segment, '多标签客群允许重叠'),
      ],
      [
        '单击 KPI 联动高亮，双击进入居中放大详情。',
        '真实聚合指标与规则推导候选明确区分。',
        '当前样本仅覆盖广东侧用户访问香港网络。',
      ],
    ))
  ...
  catalog.set('traffic-trend', story(
    'ACTIVE USERS & ROAMING TRAFFIC',
    '活跃用户与漫游流量趋势',
    '按日展示当前周期的活跃用户、话单记录和漫游流量。',
    [
      metric('活跃用户', number(summary.activeUsers), '当前筛选范围内的去重样本用户'),
      metric('样本话单', number(summary.recordCount), '当前筛选范围内的真实脱敏记录'),
      metric('漫游流量', formatBytes(summary.totalBytes ?? 0, 2), 'RG 使用量统一按 bytes 聚合'),
    ],
    [
      '趋势用于解释样本规模变化。',
      '样本抽取边界不外推为完整生产业务量。',
      '后续可由 DataEase 或 API 适配器替换。',
    ],
  ))

  catalog.set('experience-signals', story(
    'EXPERIENCE BEHAVIOR SIGNALS',
    '用户体验行为信号',
    '基于月度行为规则形成可重叠的运营候选群体。',
    (dashboard.value.experienceSignals ?? []).slice(0, 3)
      .map((item) => metric(item.name, `${number(item.value)} 人`, '规则候选，不代表身份或满意度认定')),
    [
      '候选群体之间可以重叠。',
      '体验信号不直接代表用户主观感受。',
      '正式运营前需结合套餐与合规授权数据。',
  ...
  catalog.set('flow-topology', story(
    'SAMPLE COVERAGE & USAGE SCENARIO',
    '样本覆盖与跨境使用场景',
    '当前真实样本只覆盖广东侧用户访问香港网络。',
    (dashboard.value.flows ?? [])
      .map((flow) => metric(
        '粤 → 港',
        formatBytes(flow.totalBytes ?? 0, 2),
        `${number(flow.userCount)} 位用户 · ${number(flow.completedCount)} 条记录`,
      )),
    [
      '单侧样本不表达为双向业务量。',
      '未提供的对侧数据标记为覆盖边界。',
  ...
  catalog.set('opportunities', story(
    'AUDIENCE OPPORTUNITIES',
    '重点客群与运营机会',
    '将真实使用聚合转化为体验改善与经营候选。',
    [
      metric('高用量重点用户', `${number(value.highUsageUsers)} 人`, '用量提醒、大流量包与服务保障候选'),
      metric('低活跃召回候选', `${number(value.recallCandidates)} 人`, '激活引导与回归权益候选'),
      metric('价值提升候选', `${number(value.growthCandidates)} 人`, '套餐匹配与阶梯权益候选'),
    ],
    [
      '候选规模来自月度多标签行为规则。',
      '实际触达必须经过业务校验。',
  ...
  catalog.set('risk-ranking', story(
    'HIGH-USAGE CUSTOMER FOCUS',
    '高用量重点关怀用户 TOP 10',
    '按当前周期用户总流量排序的脱敏聚合画像。',
    (dashboard.value.highValueRecords ?? []).slice(0, 3)
      .map((record) => metric(
        record.phone,
        formatBytes(record.totalBytes, 2),
        `${record.activeDays} 个活跃日 · 流量贡献 ${percent(record.trafficShare)}`,
      )),
    [
      '单击打开侧边画像，双击进入居中放大画像。',
      '流量排序不代表收入、信用或社会身份。',
      '页面不发布号码明文、IMSI、IP 或位置明细。',
    ],
  ))

  catalog.set('recent-focus', story(
    'RECENT CUSTOMER FOCUS',
    '近期重点关怀用户',
    '在高用量用户中按最后活跃日期排序。',
    (dashboard.value.recentFocusUsers ?? []).slice(0, 3)
      .map((record) => metric(
        record.lastActiveDay,
        record.phone,
        `${record.activeDays} 个活跃日 · ${record.status}`,
      )),
    [
      '近期活跃只表示样本中的最后记录日期。',
      '列表不冒充实时告警。',
  ...
  catalog.set('experience-path', story(
    'EXPERIENCE IMPROVEMENT',
    '用户体验改善路径',
    '从激活体验、套餐适配到重点用户服务保障。',
    [
      metric('激活体验', `${number(value.recallCandidates)} 人`, '低活跃候选的首次使用引导'),
      metric('套餐适配', `${number(value.growthCandidates)} 人`, '匹配日包、周包或阶梯权益'),
      metric('服务保障', `${number(value.highUsageUsers)} 人`, '高用量用户预警、客服与网络保障'),
    ],
    [
      '改善路径是建议动作，不是已发生效果。',
      '每一步需要触达权限和效果评估。',
  ...
  catalog.set('value-conversion', story(
    'COMPANY VALUE CONVERSION',
    '公司价值转化',
    '从体验改善延伸到客户价值提升的候选路径。',
    [
      metric('头部流量贡献', percent(value.headTrafficShare), 'Top 10% 高用量用户的真实聚合贡献'),
      metric('企业场景候选', `${number(value.enterpriseCandidates)} 人`, '需结合企业身份与授权数据验证'),
      metric('高频商旅候选', `${number(value.frequentTravelCandidates)} 人`, '需结合出行与套餐数据验证'),
    ],
    [
      '价值路径包括召回、套餐升级、企业试点和重点服务。',
      '机会规模不等同于实际收入。',
      '正式评估需补充触达与转化结果。',
    ],
  ))

  catalog.set('pipeline-health', story(
    'DATA TRUST & SCOPE',
    '数据可信度与适用边界',
    '展示样本、字段、映射、时间质量和对侧覆盖状态。',
    (dashboard.value.pipeline ?? []).slice(0, 3)
      .map((item) => metric(item.label, item.latency, item.description)),
    [
      '绿色状态表示当前检查通过。',
      '警示状态表示数据适用边界。',
      '对侧样本未提供，不开展双边结算推断。',
...
const activeDetail = computed(() => {
  if (!routeDetailId.value || panelFocusId.value) return null
...
const isFocusOpen = computed(() => Boolean(activeDetail.value || panelFocusId.value))
</script>

<template>
  <div class="dashboard-shell" :class="{ 'is-focus-open': isFocusOpen }">
    ...
    <div v-if="loading && !dashboard" class="loading-state" aria-live="polite">
      ...
      <strong>正在装载脱敏聚合数据</strong>
      <small>StaticSampleDataAdapter · v2026.08.02</small>
    </div>

    <div v-else-if="error && !dashboard" class="error-state" role="alert">
      <strong>数据装载失败</strong>
      <span>{{ error }}</span>
    ...
    >
      <section
        class="kpi-grid"
        :class="{ 'tour-highlight': activeStep === 'kpis' }"
        aria-label="核心洞察指标"
      ...
      <section class="analytics-grid">
        <div class="left-column">
          <PanelFrame
            ...
            detail-id="traffic-trend"
            :focused="panelFocusId === 'traffic-trend'"
            @open-detail="openDetail"
            @close-detail="goHome"
          >
          ...
          <PanelFrame
            ...
            detail-id="experience-signals"
            :focused="panelFocusId === 'experience-signals'"
            @open-detail="openDetail"
            @close-detail="goHome"
          >
            <template #header><span class="panel-badge">月度多标签</span></template>
            <ExperienceSignals
              :data="dashboard.experienceSignals"
        ...
        <div class="center-column">
          <PanelFrame
            ...
            :active="activeStep === 'flow'"
            detail-id="flow-topology"
            :focused="panelFocusId === 'flow-topology'"
            @open-detail="openDetail"
            @close-detail="goHome"
          >
          ...
          <PanelFrame
            ...
            detail-id="opportunities"
            :focused="panelFocusId === 'opportunities'"
            @open-detail="openDetail"
            @close-detail="goHome"
          >
        ...
        <div class="right-column">
          <PanelFrame
            ...
            :active="activeStep === 'ranking'"
            detail-id="risk-ranking"
            :focused="panelFocusId === 'risk-ranking'"
            @open-detail="openDetail"
            @close-detail="goHome"
          >
            <template #header><span class="panel-badge">流量贡献</span></template>
            <HighValueRanking
              :records="dashboard.highValueRecords"
              @select="selectRecord"
              @open-detail="openDetail"
            />
          </PanelFrame>
          ...
          <PanelFrame
            ...
            compact
            detail-id="recent-focus"
            :focused="panelFocusId === 'recent-focus'"
            @open-detail="openDetail"
            @close-detail="goHome"
          >
            <template #header>
              <span class="queue-count">{{ dashboard.recentFocusUsers.length }} 个脱敏样本</span>
            </template>
            <RecentFocusList
              :records="dashboard.recentFocusUsers"
              @select="selectRecord"
              @open-detail="openDetail"
            />
          </PanelFrame>
      ...
      <section class="bottom-grid">
        <PanelFrame
          ...
          :active="activeStep === 'experience'"
          compact
          detail-id="experience-path"
          :focused="panelFocusId === 'experience-path'"
          @open-detail="openDetail"
          @close-detail="goHome"
        >
        ...
        <PanelFrame
          ...
          :active="activeStep === 'value'"
          compact
          detail-id="value-conversion"
          :focused="panelFocusId === 'value-conversion'"
          @open-detail="openDetail"
          @close-detail="goHome"
        >
        ...
        <PanelFrame
          ...
          :active="activeStep === 'pipeline'"
          compact
          detail-id="pipeline-health"
          :focused="panelFocusId === 'pipeline-health'"
          @open-detail="openDetail"
          @close-detail="goHome"
        >
    ...
    <div v-if="tour.state.value !== 'idle'" class="tour-status" aria-live="polite">
      ...
      <div>
        <small>演示导览</small>
        <strong>{{ tour.stepIndex.value + 1 }} / {{ tour.totalSteps }} · {{ activeStep }}</strong>
      </div>
    </div>
