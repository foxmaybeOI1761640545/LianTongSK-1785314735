...
const option = computed(() => ({
  color: colors,
  tooltip: { trigger: 'item', formatter: '{b}<br/>{c} 个关注信号 · {d}%', backgroundColor: 'rgba(4, 15, 30, .96)', borderColor: 'rgba(69, 230, 255, .35)', textStyle: { color: '#edf8ff' } },
  ...
  graphic: [{ type: 'text', left: '26%', top: '44%', style: { text: `${props.data.reduce((sum, item) => sum + item.value, 0).toLocaleString('zh-CN')}\n信号`, textAlign: 'center', fill: '#edf8ff', font: '600 12px sans-serif', lineHeight: 18 } }],
}))
useEChart(chartElement, option)
</script>

<template><div ref="chartElement" class="chart-canvas" data-testid="distribution-chart" aria-label="数据质量关注信号分布图"></div></template>
