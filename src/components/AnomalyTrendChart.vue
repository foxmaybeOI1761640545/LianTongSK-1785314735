...
const option = computed(() => ({
  ...
  tooltip: {
    trigger: 'axis',
    valueFormatter: (value) => `${Number(value).toLocaleString('zh-CN')} 条账期外记录`,
  ...
  yAxis: {
    ...
    axisLabel: { color: '#6f91aa', fontSize: 10, formatter: '{value}条' },
    splitLine: { lineStyle: { color: 'rgba(80, 139, 177, .12)' } },
  },
  series: [{
    type: 'line', smooth: true, showSymbol: false, data: props.data.map((item) => item.anomalyCount),
...
<template><div ref="chartElement" class="chart-canvas" data-testid="anomaly-chart" aria-label="账期外记录数量趋势图"></div></template>
