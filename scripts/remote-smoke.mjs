...
try {
  ...
  await page.locator('[data-kpi="traffic"]').getByText('23.7 GB', { exact: true }).waitFor()
  ...
  const firstMaskedPhone = await page.locator('[data-testid="high-value-ranking"] .rank-main strong').first().textContent()
  if (!/^1\d{2}\*{4}\d{4}$/.test(firstMaskedPhone ?? '')) throw new Error(`Expected a masked real phone, received ${firstMaskedPhone}`)
  await page.locator('[data-testid="settlement-panel"]').getByText('十进制流量 × ¥5 / GB', { exact: true }).waitFor()
  const qualityChecks = page.locator('[data-testid="quality-check-list"] > li:not(.quality-empty)')
  const qualityCheckCount = await qualityChecks.count()
  if (qualityCheckCount !== 5) throw new Error(`Expected 5 quality checks, received ${qualityCheckCount}`)
  ...
  if ((await page.locator('canvas').count()) < 2) throw new Error('Expected at least 2 ECharts canvases')
