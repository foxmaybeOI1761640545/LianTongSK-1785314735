...
describe('StaticSampleDataAdapter', () => {
  ...
  it('returns the reconciled full-sample totals with a visible source boundary', async () => {
    ...
    expect(first.settlement.rateCnyPerGB).toBe(5)
    expect(first.summary.hkPayable).toBeCloseTo(118.42918098)
  ...
  it('keeps range metrics and their provenance signals internally consistent', async () => {
    ...
    const crossPeriodSignal = data.anomalyDistribution.find((item) => item.id === 'cross-period')
    ...
    expect(crossPeriodSignal.unit).toBe('条记录')
    expect(data.anomalyDistribution.find((item) => item.id === 'exact-duplicate').name).toBe('完全重复冗余')
  })
  ...
  it('shows that the supplied sample has no reverse-direction records', async () => {
    ...
    expect(hkToGd.summary.hasSample).toBe(false)
    expect(hkToGd.kpis.every((kpi) => kpi.displayValue === '未覆盖')).toBe(true)
    expect(hkToGd.highValueRecords).toHaveLength(0)
    expect(hkToGd.flows.find((flow) => flow.direction === 'HK_TO_GD').totalBytes).toBeNull()
  })
})
